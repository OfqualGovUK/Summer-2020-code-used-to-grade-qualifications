################################################################################################
##### Code to run the DCP approach - AS - updated 06/07/20 ######
###############################################################################################
...
ASmodeloutput<-ASDCPmodel(currentyear_nonprivate, yearminus1and2and3, outputs=TRUE)



################ MODEL FUNCTION - RUN BELOW FIRST ########################################
...
#DCP model function
ASDCPmodel<-function(currentdata, previousyear, outputs=FALSE){
  
  #### X2 - GENERATE PRIOR-ATTAINMENT CUT_OFFS
  #only keep 17 year old students for predictions with at least 3 GCSEs and prior attainment data
  previousyearsub<-subset(previousyear, NGCSEsTaken>2&YearEndAge==17&!is.na(NormalisedMeanGCSE_UK))
  ...
  predictionmatrix<-previousyearsub%>%group_by(SubjectGroup, equaldecile)%>%
    summarise(NcandsPM=length(UniqueCandidateIdentifier),
              PMA=length(UniqueCandidateIdentifier[SubjectLevelGrade%in%c( "A")])/NcandsPM,
              PMB=length(UniqueCandidateIdentifier[SubjectLevelGrade%in%c( "A", "B")])/NcandsPM,
              PMC=length(UniqueCandidateIdentifier[SubjectLevelGrade%in%c("A","B","C")])/NcandsPM,
              PMD=length(UniqueCandidateIdentifier[SubjectLevelGrade%in%c("A","B","C","D")])/NcandsPM,
              PME=length(UniqueCandidateIdentifier[SubjectLevelGrade%in%c("A","B","C","D","E")])/NcandsPM,
              PMU=length(UniqueCandidateIdentifier[SubjectLevelGrade%in%c("A","B","C","D","E","U")])/NcandsPM)%>%
    ungroup()
  ...
  if (outputs==TRUE){
    ...
    outputD25<-outputD25%>%mutate(
      ...
      select(QualificationLevel, Type, SubjectGroup, NMatchHistoricalCandidates=NcandsPM,
             Decile=equaldecile, GradeA, GradeB, GradeC, GradeD, GradeE)
  ...
  #remove mean GCSE for candidates with less than 3 GCSEs and not 17 to match for predictions
  previoussub$NormalisedMeanGCSE_UK[previoussub$NGCSEsTaken<3|previoussub$YearEndAge!=17]<-NA
  ...
  previouscent<-previoussub%>%
    ...
    summarise(totalcands_centprevious=length(UniqueCandidateIdentifier),
              ...
              PropA_centprevious=(length(UniqueCandidateIdentifier[SubjectLevelGrade%in%c( "A")]))/(totalcands_centprevious),
              PropB_centprevious=(length(UniqueCandidateIdentifier[SubjectLevelGrade%in%c( "A", "B")]))/(totalcands_centprevious),
              PropC_centprevious=(length(UniqueCandidateIdentifier[SubjectLevelGrade%in%c("A","B","C")]))/(totalcands_centprevious),
              PropD_centprevious=(length(UniqueCandidateIdentifier[SubjectLevelGrade%in%c("A","B","C","D")]))/(totalcands_centprevious),
              PropE_centprevious=(length(UniqueCandidateIdentifier[SubjectLevelGrade%in%c("A","B","C","D","E")]))/(totalcands_centprevious),
              PropU_centprevious=(length(UniqueCandidateIdentifier[SubjectLevelGrade%in%c("A","B","C","D","E","U")]))/(totalcands_centprevious),
  ...
  if (outputs==TRUE){
    ...
    outputD4<-outputD4%>%mutate(ActualHistA=round2(PropA_centprevious*100,2),
      ...
      select(QualificationLevel, Type, AwardingOrganisation,SubjectGroup, CentreNo,
             ...
             ActualHistA,ActualHistB, ActualHistC, ActualHistD, ActualHistE)
  ...
  #remove mean GCSE score for those with less than 3 GCSEs and not 17 to only produce predictions for matched candidates
  currentdatasub$NormalisedMeanGCSE_UK[currentdatasub$NGCSEsTaken<3|currentdatasub$YearEndAge!=17]<-NA
  ...
  Alldata<-Alldata%>%mutate(PredAcurrent=PropA_centprevious+minmatch*(PredA_centcurrent-PredA_centprevious),
  ...
  Alldata<-Alldata%>%group_by(SubjectGroup, CentreNo, totalcands_centcurrent, totalcands_centprevious, matchedcands_centcurrent)%>%
    summarise(PredA=ifelse(PredAcurrent<0, 0, ifelse(PredAcurrent>1, 1, PredAcurrent)),
  ...
  if (outputs==TRUE){
    ...
    outputD6<-outputD6%>%mutate(PredA=round2(PredA*100,2),
