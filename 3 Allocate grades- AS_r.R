################################################################################
##### Functions for allocating grades to candidates AS - updated 06/07/20 ######
################################################################################
...
#filter to As only
TargetDistributions<-subset(TargetDistributions, Qualification.Level=="AS")

#include list of subjects which should not be adjusted to meet predictions
ASSmallSubList<-TargetDistributions$Subject.Grouping[TargetDistributions$Total.Matched<500]
...
currentyear_nonprivate_imputedmarks<-Imputemarks(currentyear_private, currentyear_nonprivate, ASmodeloutput, outputs=TRUE)

# 2. calculate grade boundaries based on marks and required cumulative frequencies except subjects in small subject list 
ASgradeboundaries<-calculategradeboundaries(currentyear_nonprivate_imputedmarks, TargetDistributions, outputs=TRUE)

# 3. impute grades based on marks and grade boundaries except subjects in small subject list 
currentyear_nonprivate_imputedgrades<-Imputegradebasedonmark(currentyear_private, currentyear_nonprivate_imputedmarks,ASgradeboundaries, outputs=FALSE)
...
Imputemarks<-function(currentdata_private, currentdata, modeloutput, outputs=FALSE){
  ...
  candidatemarks<-candidatemarks%>%mutate(rankperc=(rank_noprivate-0.5)/totalcands,
                                          ...
                                          UnadjustedGrade=dplyr::recode(maxgrade, "5"="A","4"="B","3"="C", "2"="D", "1"="E", "0"="U")
                                          #remove unneeded variables
  )%>%select(-c(totalcands, rankperc, maxgrade, cumperc, cumpercplus1,totalcands_centprevious,
                totalcands_centcurrent, matchedcands_centcurrent, 
...
calculategradeboundaries<-function(candidatemarks, target, outputs=FALSE){
  ...
  boundaries<-data.frame(SubjectGroup=character(),NMatchedCands=numeric(), MarkCutE=numeric(), MarkCutD=numeric(),MarkCutC=numeric(),MarkCutB=numeric(),MarkCutA=numeric(), stringsAsFactors = FALSE)
  
  #remove small subjects
  candidatemarks<-subset(candidatemarks, !SubjectGroup%in%ASSmallSubList)
  
  #filter to macthed england only candidates and assign percentile rank
  candidatemarks<-candidatemarks%>%
    filter(YearEndAge==17&!is.na(NormalisedMeanGCSE_England)&NGCSEsTaken>2&!is.na(CandidateMark)&Country=="England")%>%
  ...
  for(i in 1:length(subnames)){
    ...
    #lookup target distribution from predictions
    ...
    #calculate boundaries as the nearest candidate mark at prediction percentile
    ...
    boundaries[nrow(boundaries)+1,]<-t(c(subnames[i],NMatched,Ebound,Dbound,Cbound,Bbound,Abound))
...
Imputegradebasedonmark<-function(currentdata_private, candidatemarks, gradeboundaries, outputs=FALSE){
  ...
  candidatemarks_smallsubs<-subset(candidatemarks, SubjectGroup%in%ASSmallSubList)
  candidatemarks_othersubs<-subset(candidatemarks, !SubjectGroup%in%ASSmallSubList)
  ...
  for(i in 1:length(subnames)){
    ...
    labels<-c("U", "E", "D", "C", "B", "A")
    
    #replace bin numbers with grade labels
    markssub$ImputedGrade<-dplyr::recode(markssub$ImputedGrade, "6"=labels[6], "5"=labels[5], "4"=labels[4], "3"=labels[3], "2"=labels[2], "1"=labels[1])
